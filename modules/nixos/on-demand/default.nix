...
in {
  options = {
    icebox.static.system.minecraft-server = {
      ...
      onDemand = mkOption {
        ...
        description = "A daemon that monitors the actual Minecraft server. If the server is up and there is no player, then daemon shuts the server down. If the server is down and there is an incoming connection, daemon starts the actual server. Otherwise, daemon does nothing.";
      };
  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    (mkIf (cfg.onDemand.enable) {
      ...
      icebox.static.system.minecraft-server.serverProperties.server-ip = "127.0.0.1";
      ...
      systemd.services = {
        ...
        proxy-minecraft-server = {
          ...
          requires = ["minecraft-server.target" "proxy-minecraft-server.socket"];
        ...
        minecraftd = let
          ...
          postStopScript =
            pkgs.writeShellScript "minecraftd-poststop"
      ...
      systemd.sockets.proxy-minecraft-server = {
        ...
        listenStreams = ["${cfg.serverIp}:${toString cfg.serverProperties.server-port}"];
